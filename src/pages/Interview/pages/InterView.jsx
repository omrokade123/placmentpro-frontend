...
const Interview = () => {
  ...
  const handleSchedule = async () => {
    const data = await scheduleMockInterview(reportId);
    if (!data || !data.interview) {
      console.error("Interview scheduling failed");
      return;
    }

    navigate(`/interview/mock-interview/${data.interview._id}`);
  };

  const handleResume = () => {
    navigate(`/interview/mock-interview/${session._id}`);
  };

  const handleViewFeedback = () => {
    navigate(`/interview/interview-feedback/${session._id}`, {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const interviewSession = await checkInterviewSession(reportId);
      ...
      <div className="max-w-6xl mx-auto space-y-8">
          ...
          <CardContent className="pt-6">
            <div className="flex flex-wrap gap-3">
              {report.skillGaps.map((gap, i) => (
                <Badge
                  key={i}
                  className="bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border-2 border-red-300 dark:border-red-700 text-sm px-3 py-2"
                >
                  {gap.skill}
                </Badge>
              ))}
        ...
        <Tabs defaultValue="technical" className="space-y-4">
          <TabsList className="grid grid-cols-3 w-full h-12 bg-gray-200 dark:bg-gray-800 p-1 rounded-lg">
            <TabsTrigger value="technical" className="rounded-md font-semibold">
              ⚙️ Technical
            </TabsTrigger>
            <TabsTrigger
              value="behavioral"
            ...
            <TabsTrigger value="roadmap" className="rounded-md font-semibold">
              🗺️ Roadmap
            </TabsTrigger>
          </TabsList>
                ...
                <Accordion type="single" collapsible className="space-y-2">
                  {report.technicalQuestions.map((q, i) => (
                    <AccordionItem
                      key={i}
                      value={`tech-${i}`}
                      className="border-2 border-gray-200 dark:border-gray-700 rounded-lg px-4"
                    >
          ...
          <TabsContent value="roadmap">
            <div className="grid gap-4">
              {report.preparationPlan.map((day) => (
                <Card
                  key={day.day}
                  className="border-2 border-gray-200 dark:border-gray-800 hover:shadow-lg transition-all"
                >
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
          {!session && (
            <Button
              ...
              className="h-12 gap-2 bg-purple-600 hover:bg-purple-700 text-white"
            >
              <Calendar size={18} />
          ...
          {session?.status === "scheduled" && (
            <Button
              ...
              className="h-12 gap-2 bg-indigo-600 hover:bg-indigo-700 text-white"
            >
